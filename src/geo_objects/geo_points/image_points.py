...
class ImagePoint(BasicPoint):
    ...
    def get_popup_info(self):
        ...
        """
        details = [
            '<div style="width: 50vw; height: auto; max-height: 70vh; overflow-y: auto; '
            'padding: 20px; box-sizing: border-box; font-size: 1.2rem;">',
            '<strong style="font-size: 1.5rem; margin-bottom: 10px;">Image Details</strong><br>'
        ]
        if self.time:
            details.append(
                f'<span style="display: block; margin-top: 10px;">Time: {self.time.strftime("%H:%M:%S")}</span>')
        details.append(f'<span style="display: block; margin-top: 5px;">Filename: {self._file_name}</span>')
        if self.elev is not None:
            details.append(f'<span style="display: block; margin-top: 5px;">Elevation: {round(self.elev, 1)} M</span>')
        if self._additional_info:
            details.append(f'<span style="display: block; margin-top: 5px;">Info: {self._additional_info}</span>')
        if self._image_url:
            details.append(
                f'<img src="{self._image_url}" alt="{self._file_name}" style="width: 100%; '
                'max-width: 48vw; height: auto; border-radius: 10px; margin-top: 20px;">'
            )
        details.append('</div>')
        return ''.join(details)
