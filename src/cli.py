...
@click.option('--map-tile', type=str, required=False, multiple=True, default=['OpenStreetMap'],
              help='The map tile providers to use. Can be specified multiple times.')
...
def main(gpx_file, output_map, map_tile, map_attr, output_report, picture_folder):
    ...
    try:
        ...
        # Ensure the number of map tiles and attributions match
        if len(map_tile) != len(map_attr):
            click.echo("The number of --map-tile and --map-attr options must be the same.", err=True)
            return
